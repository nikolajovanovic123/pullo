...
window.onload=function()
{
    ...
    function showProducersList(producers){
        ...
        kategorije=producers;
        //dohvatiPodatke("products", showProducts);
        dohvatiPodatke("colors", showColors);
        $(".producer").change(filterChange);
    }
    function showColors(colors)
    {
        ...
        for(let c of colors)
        {
            html+=`
            <li class="list-group-item">
              <input type="checkbox" id="${c.id}" name="color" class="color" value="${c.id}"/> ${c.color}
            </li>
            `;
        }
    ...
    function filterColor(products)
    {
        ...
        $(".color:checked").each(function(){
            colorIds.push(parseInt($(this).val()));
        });
        if(colorIds.length != 0)
        {
            return products.filter(p => p.colors.some(y => colorIds.includes(y)));
        }
        else return products;
    }
